...
- `LLM_BASE_URL` - LLM endpoint (default: `http://127.0.0.1:8000/v1` for oMLX, `:11434/v1` for Ollama)
- `LLM_API_KEY` - LLM authentication (default: `omlx` for oMLX, `ollama` for Ollama)
...
- Local LLM server running (oMLX on `127.0.0.1:8000/v1` or Ollama on `:11434/v1`)
- Network access to:
