...
def test_normal_flow_still_works(monkeypatch):
    ...
    monkeypatch.setattr(triage_score, "llm",
                        lambda msgs, max_tokens=400: "Dette er en normal BLUF.")
...
def test_cap_truncates_tail_when_over_budget(monkeypatch):
    ...
    K = len(sent_indices)
    assert sent_indices == list(range(1, K + 1)), \
    ...
    assert K < n, \
        f"cap={cap} expected to trim at least one item from {n}; got K={K}"
