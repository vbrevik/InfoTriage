...
def _group_by_cnr_and_ccir(
    rows: list[dict], ccir_order: list[tuple[str, str]]
) -> dict[str, list[dict]]:
    ...
    *,
    ...
    if cat_i:
        ...
        for cl in clusters:
            ...
            tag = (
                f"  _({len(cl['items'])} kilder: {', '.join(srcs)})_"
                if len(cl["items"]) > 1
                else ""
            )
    ...
    for priority_label, group_rows in [
        ("CAT_I", cat_i),
        ("CAT_II", by_cnr["CAT_II"]),
        ("ROUTINE", by_cnr["ROUTINE"]),
    ]:
        if not group_rows:
            continue
    ...
    routine_no_ccir = [
        ...
        if (r.get("ccir") or "none").lower() == "none" and r.get("cnr") in ("none", "")
    ]
    if routine_no_ccir:
...
def render_bluf(
    ...
    try:
        print(
            f"  …generating BLUF for {ccir_id or 'topic'} ({len(ctx_blocks_copy)} items, "
            f"~{_estimate_tokens(prompt)} tok)",
            file=sys.stderr,
            flush=True,
        )
...
def render_cluster(
    ...
    *,
    ...
    no_ccir = [
        r for r in enrichment_rows if (r.get("ccir") or "none").lower() == "none"
    ]
    if no_ccir:
