...
# render_wikilinked moved to contracts._wikilink (2026-08-01) so apps/wiki can
# link entities without a cross-app import; re-exported here for existing callers.
from contracts import render_wikilinked  # noqa: E402,F401
...
def write_entity_graph_from_store(
    ...
    *,
    ...
    Queries Store.get_active_entities() so the graph shows the most active
    canonical entities from the Postgres system of record, enriched with
    temporal stats and CCIRs, rather than the in-row entity projection.
    """
    vault_path.mkdir(parents=True, exist_ok=True)
    filepath = vault_path / filename

    entities = store.get_active_entities(limit=ENTITY_GRAPH_ACTIVE_LIMIT)
    ...
    *,
    ...
    Args:
        ...
        store: Optional Store instance. When provided, the Entity Graph.md is
               generated by querying the store directly via get_active_entities().
